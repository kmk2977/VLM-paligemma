...
class GemmaRotaryEmbedding(nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, device=None):
        ...
        #calculate the theta according to the formula theta_i = base^(wi/dim) where i=0,1,2,3,....,dim // 2
    ...
    def forward(self, x, position_ids, seq_len=None):
        ...
        #used for mixed precession(do not have to work with float32 values) gpus support 16 bit also which is more faster
        with torch.autocast(device_type=device_type, enabled=False):
            #multiply each theta by the position (which is the argument of the sin cos functions)
...
def apply_rotary_pos_emb(q, k, cos, sin, unsqueeze_dim=1):
    ...
    #apply formula(34) of rotary positional encoding paper
